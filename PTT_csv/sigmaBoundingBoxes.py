...
# connection = pymysql.connect(host='localhost',port=8080, user='root', password='Archit@28', db='gmfldesktop12')
...
def get_adaptive_sigma_refinement(length_percent):
    ...
    if 1 <= length_percent < 10:
        sigma_multiplier = 0.8      # Less aggressive for very small defects
...
# df = pd.read_pickle("Archit\PTT_csv\GMFL_12inch_24-01-2025_PTT(2)/3.pkl")
    ...
    try:
        ...
        for col_idx in range(start_sensor, end_sensor + 1):
            ...
            if submatrix.iloc[:, col_idx - start_sensor].max() > adaptive_sigma:
                valid_columns += 1
            if valid_columns/(end_sensor - start_sensor + 1) < 0.5:
        ...
        if (adjusted_threshold <= max_value <= max_of_all):
            ...
            if depth_val > 1 and width > 1 and length > 1:
                ...
                figx112.add_annotation(
                    ...
                    y=start_sensor - 1,
                    text=f"{defect_counter}({classification.split()[0]})<br>Sensor: {start1_1} to {end1_1}",
                    showarrow=False,
...
with (connection.cursor() as cursor):
    for i in finial_defect_list:
        ...
        query_defect_insert = """
            INSERT INTO defect_local_hm(runid, start_index, end_index, start_sensor, end_sensor, absolute_distance,
