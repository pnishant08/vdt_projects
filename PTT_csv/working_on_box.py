...
Read Csv file 
"""
# df = pd.read_pickle("D:/VDT/PTT_csv/GMFL_12inch_28-01-2025_PTT(3)/3.pkl")
df = pd.read_pickle("C:/Users/pnish/Desktop/PTT_CSV/VDT_WORK_UPDATE/Nishant/PTT_csv/GMFL_12inch_24-01-2025_PTT(2)/3.pkl")


def calculate_width(mask, div_factor):
    if isinstance(mask,pd.DataFrame):
        mask = mask.values
    col_activty = np.sum(mask, axis=0)
    active_cols = np.where(col_activty > 0)[0]
    if(len(active_cols) == 0):
        return 0
    width = (active_cols[-1] - active_cols[0] + 1) * div_factor
    return width
#==============================================================================================================================
#                                      STATISTICAL METHODS 
#==============================================================================================================================
...
for _, row in df_sorted.iterrows():
    ...
    if start_sensor == end_sensor:
        ...
        try:
            submatrix = df3_raw.iloc[start_reading:end_reading + 1, start_sensor:end_sensor + 1]
        ...
        try:
            ...
            if (threshold_value <= max_value <= max_of_all):
                ...
                def process_submatrix(df_diff):
                    ...
                    # expand_by = 1  # You can increase to 2 if needed

                    # # Expand the box by 'expand_by' sensors on each side
                    # new_start_sensor = max(0, start1_1 + num_cols_to_remove_start - expand_by)
                    # new_end_sensor = min(df_diff.shape[1] - 1, end1_1 - num_cols_to_remove_end + expand_by)
                ...
                if depth_val > 5 and width > 3 and length > 3:
                    ...
                    #width_gradient = quantreg_width(modified_df)
                    ...
                    #      "length_percent": length_percent, "breadth": width_hybrid, 'width_new': width_slope, 'width_new2': round(width_1_only, 0),
...
with (connection.cursor() as cursor):
    for i in finial_defect_list:
        ...
        query_defect_insert = """
            INSERT INTO interptable(runid, start_index, end_index, start_sensor, end_sensor, absolute_distance,
