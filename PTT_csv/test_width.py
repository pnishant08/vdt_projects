...
# from interpolation_utils import process_csv_with_interpolation_row
# from width_processing_utils import interpolate_matrix, apply_mad_filtering, calculate_width
from width_processing_utils import interpolate_matrix, apply_mad_filtering, calculate_width

# from ks_testing.ks_filter import is_significant_defect_matrix
...
# df = pd.read_pickle("C:/Users/pnish/Desktop/PTT_CSV/VDT_WORK_UPDATE/Nishant/PTT_csv/GMFL_12inch_22-01-2025_PTT(6)/3.pkl")
...
# Adaptive positive sigma using IQR
Q1 = df2.quantile(0.25)
Q3 = df2.quantile(0.75)
IQR = Q3 - Q1
std_series = df2.std(skipna=True)

# Avoid divide-by-zero
adaptive_sigma = 0.5 + 0.5 * (IQR / (std_series.replace(0, np.nan)))
adaptive_sigma = adaptive_sigma.fillna(0.75)  # fallback to 0.75 if NaN

# Apply adaptive sigma to mean
mean_plus_1sigma = []
for i, data1 in enumerate(mean1):
    sigma1 = data1 + adaptive_sigma.iloc[i] * standard_deviation[i]
    mean_plus_1sigma.append(sigma1)
...
for _, row in df_sorted.iterrows():
            ...
            if (threshold_value <= max_value <= max_of_all):
                ...
                def process_submatrix(df_diff):
                    if df_diff.isnull().values.any():
                        return None
                #    applying ks test here 
                    # if not is_significant_defect_matrix(df_diff):
                    #    print("KS test failed — skipping this submatrix")
                    #    return None
 ...
 # ------------------     1. Normalize each row using min-max normalization before row stdev --------------------------
                    # df_diff = df_diff.apply(lambda row: (row - row.min()) / (row.max() - row.min()) if row.max() != row.min() else row, axis=1)
                    

                    # Median Absolute Deviation instead of Standard Deviation
                    df_diff_median = df_diff.median(axis=1)
                    mad = df_diff.apply(lambda row: np.median(np.abs(row - np.median(row))), axis=1)

                    # You can experiment with the multiplier (1.5–2.5)
                    mad_threshold = df_diff_median + 2.0 * mad
                    mad_threshold_series = pd.Series(mad_threshold, index=df_diff.index)
                    
                    # Apply MAD-based thresholding
                    df_result = df_diff.apply(lambda row: row.map(lambda x: 1 if x > mad_threshold_series[row.name] else 0), axis=1)
                    ...
                    # Tolerance and correction to match manual width measurements
                    # tolerance_gap = 1  # You can later fine-tune this value
                    # correction_factor = 1.05  # You can fine-tune this between 1.03 - 1.10 based on actual data
                    
                    # width_1_only = round((num_cols_with_ones + tolerance_gap) * div_factor * correction_factor)
                     # Dynamic correction based on defect size
                    # if num_cols_with_ones < 10:
                    #     correction_factor = 1.08  # Increase correction for small widths
                    # elif num_cols_with_ones < 20:
                    #     correction_factor = 1.05  # Medium correction
                    # else:
                    #     correction_factor = 1.02  # Small correction for large widths
                    
                    # tolerance_gap = 1  # Allow a small connection gap
                    
                    # width_1_only = round((num_cols_with_ones + tolerance_gap) * div_factor * correction_factor)
         
                    # Apply generous tolerance and stronger correction

                    tolerance_gap = 4  # Allow up to 4 zero columns as connection gap
                    
                    if num_cols_with_ones < 20:
                        correction_factor = 1.30  # Stronger correction for small defects
                    elif num_cols_with_ones < 40:
                        correction_factor = 1.20  # Medium correction
                    else:
                        correction_factor = 1.10  # Slight correction for larger defects
                ...
                # modified_df = process_csv_with_interpolation_row(df_duplicate, x1, y1, z1, div_factor)

                
                # interpolated_df = interpolate_matrix(df_duplicate)
                # filtered_df = apply_mad_filtering(interpolated_df)
                # Use dynamic interpolation factor based on defect size
                interpolated_df_initial = interpolate_matrix(df_duplicate, interpolation_factor=3)
                filtered_matrix = apply_mad_filtering(interpolated_df_initial, mad_multiplier=0.8)
                
                defect_widths = calculate_width(filtered_matrix, div_factor)
                 
                 # If you process multiple defects, you can use:
                width_1_only = defect_widths[0]  # Use the correct index if in a loop
                 
                 # Use filtered_df in place of modified_df
                modified_df = filtered_matrix

                # # Step 1: Interpolate the matrix (before MAD)
                # interpolated_df_initial = interpolate_matrix(df_duplicate)
                
                # # Step 2: Apply MAD filtering after initial interpolation
                # filtered_df = apply_mad_filtering(interpolated_df_initial)
                
                # # Step 3: Apply interpolation again after MAD filtering
                # interpolated_df_final = interpolate_matrix(filtered_df)
                
                # # Step 4: Calculate defect widths on final interpolated matrix
                # defect_widths = calculate_width(interpolated_df_final, div_factor)
                
                # # Use the calculated width for this defect
                # width_1_only = defect_widths[0]  # If inside a loop, use the correct defect index
                
                # # Continue processing using final interpolated matrix as the defect matrix
                # modified_df = interpolated_df_final
